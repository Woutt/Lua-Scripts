...
local Library = {
    ...
    DragSpeed = 0.06,
    LockDragging = false,
    ToggleKey = lib.toggle_key,
}
Library.__index = Library

local selectedTab
pcall(function()
    if not isfolder("System-Fludex") then
        makefolder("System-Fludex")
    end
end)
...
function Library:object(class, properties)
    ...
    function methods:tween(options, callback)
        ...
        local tween = game:GetService("TweenService"):Create(localObject, ti, options); tween:Play()
...
function Library:create(options)
    ...
    updateSettings = function(property, value)
        ...
        writefile("System-Fludex/theme.lib", game:GetService("HttpService"):JSONEncode({Theme = value}))
    end
    ...
    local homeButtonIcon = homeButton:object("ImageLabel", {
        ...
        Image = "http://www.roblox.com/asset/?id=8657894580",
        Theme = {ImageColor3 = "StrongText"}
    })
    ...
    local profilePictureContainer = profile:object("ImageLabel", {
        Image = game:GetService("Players"):GetUserThumbnailAsync(LocalPlayer.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size100x100),
        Theme = {BackgroundColor3 = {"Secondary", 10}},
        AnchorPoint = Vector2.new(0, 0.5),
